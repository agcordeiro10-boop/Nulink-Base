...
const TIPOS = ['nota','ligacao','email','reuniao','whatsapp','visita','tarefa']
const ICONS: Record<string,string> = { ligacao:'📞', email:'📧', reuniao:'🤝', whatsapp:'💬', visita:'🏢', tarefa:'✅', nota:'📝' }

const emptyForm = { tipo:'nota', titulo:'', descricao:'', empresa_id:'', data_atividade: new Date().toISOString().slice(0,16) }
  ...
  return (
    <Layout>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:28 }}>
        <div><h1 style={{ fontSize:24, fontWeight:700, color:'var(--text-primary)', margin:0 }}>Histórico</h1><p style={{ color:'var(--text-muted)', fontSize:13, marginTop:4 }}>{data.length} atividades</p></div>
        <button onClick={()=>{setForm(emptyForm);setModal(true)}} style={{ display:'flex', alignItems:'center', gap:8, padding:'10px 18px', background:'var(--nulink-blue)', color:'#fff', border:'none', borderRadius:8, fontWeight:600, fontSize:14, cursor:'pointer' }}><Plus size={16}/> Registrar</button>
      </div>
      <div style={{ display:'flex', flexDirection:'column', gap:0 }}>
        ...
        {data.length===0&&<p style={{color:'var(--text-muted)',textAlign:'center',padding:40}}>Nenhuma atividade registrada.</p>}
      </div>
      {modal&&(
          ...
          <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:16, padding:32, width:480 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                <div><label style={lbl}>Tipo</label><select value={form.tipo} onChange={e=>setForm({...form,tipo:e.target.value})} style={inp}>{TIPOS.map(t=><option key={t} value={t}>{ICONS[t]} {t}</option>)}</select></div>
                <div><label style={lbl}>Empresa</label><select value={form.empresa_id} onChange={e=>setForm({...form,empresa_id:e.target.value})} style={inp}><option value="">Selecione...</option>{empresas.map(e=><option key={e.id} value={e.id}>{e.nome}</option>)}</select></div>
              </div>
              <div><label style={lbl}>Título *</label><input value={form.titulo} onChange={e=>setForm({...form,titulo:e.target.value})} style={inp}/></div>
              <div><label style={lbl}>Descrição</label><textarea value={form.descricao} onChange={e=>setForm({...form,descricao:e.target.value})} rows={3} style={{ ...inp, resize:'vertical' }}/></div>
              <div><label style={lbl}>Data/Hora</label><input type="datetime-local" value={form.data_atividade} onChange={e=>setForm({...form,data_atividade:e.target.value})} style={inp}/></div>
            </div>
