...
const emptyForm = { nome:'', gatilho:'novo_lead', acao:'criar_tarefa' }
...
export default function Automacoes() {
  ...
  async function load() {
    const { data } = await supabase.from('automacoes').select('*').order('created_at', { ascending: false })
    setData(data||[])
  }
  ...
  async function salvar() {
    ...
    setSaving(true)
    const { error } = await supabase.from('automacoes').insert({ nome: form.nome, gatilho: form.gatilho, acao: form.acao, ativa: true })
  ...
  async function toggleAtiva(a: any) {
  ...
  return (
    <Layout>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:28 }}>
        <div><h1 style={{ fontSize:24, fontWeight:700, color:'var(--text-primary)', margin:0 }}>Automações</h1><p style={{ color:'var(--text-muted)', fontSize:13, marginTop:4 }}>{data.filter(a=>a.ativa).length} ativas</p></div>
        <button onClick={()=>{setForm(emptyForm);setModal(true)}} style={{ display:'flex', alignItems:'center', gap:8, padding:'10px 18px', background:'var(--nulink-blue)', color:'#fff', border:'none', borderRadius:8, fontWeight:600, fontSize:14, cursor:'pointer' }}><Plus size={16}/> Nova Automação</button>
      </div>
      <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
        {data.map((a:any)=>(
          <div key={a.id} style={{ background:'var(--bg-card)', border:`1px solid ${a.ativa?'var(--nulink-blue)':'var(--border)'}`, borderRadius:12, padding:'16px 20px', display:'flex', alignItems:'center', gap:16, opacity:a.ativa?1:0.6 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <div style={{ fontSize:12, color:'var(--text-muted)' }}>Quando: <span style={{ color:'var(--nulink-blue)' }}>{a.gatilho?.replace(/_/g,' ')}</span> → <span style={{ color:'var(--success)' }}>{a.acao?.replace(/_/g,' ')}</span></div>
            </div>
        ...
        {data.length===0&&<div style={{ textAlign:'center', padding:60, color:'var(--text-muted)' }}><Zap size={40} style={{ opacity:0.2, marginBottom:12 }}/><br/>Nenhuma automação configurada.</div>}
      </div>
      {modal&&(
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
          <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:16, padding:32, width:460 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              <div><label style={lbl}>Nome *</label><input value={form.nome} onChange={e=>setForm({...form,nome:e.target.value})} style={inp} placeholder="ex: Follow-up após proposta"/></div>
              <div><label style={lbl}>Gatilho (quando?)</label><select value={form.gatilho} onChange={e=>setForm({...form,gatilho:e.target.value})} style={inp}>{GATILHOS.map(g=><option key={g} value={g}>{g.replace(/_/g,' ')}</option>)}</select></div>
              <div><label style={lbl}>Ação (o que fazer?)</label><select value={form.acao} onChange={e=>setForm({...form,acao:e.target.value})} style={inp}>{ACOES.map(a=><option key={a} value={a}>{a.replace(/_/g,' ')}</option>)}</select></div>
            </div>
